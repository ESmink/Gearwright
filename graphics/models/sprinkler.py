"""Installed sprinkler and brass inventory item shapes."""
...
def _body(shape: Shape, item: bool = False) -> None:
    ...
    if item:
        shape.box("threaded-inlet", (6, 12, 6), (10, 16, 10), texture="#brass", group="sprinkler-body")
    values = (
...
def _rotor(shape: Shape) -> None:
    ...
    for name, from_, to in (
        ...
        shape.box(name, from_, to, texture="#brass", group="rotor")
    for name, from_, to, origin in (
        ...
        shape.box(name, from_, to, texture="#brass", rotation_origin=origin, rotation=(0, -12, 0), group="rotor")
