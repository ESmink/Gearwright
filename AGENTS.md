...
- On Windows, compare filesystem paths only after canonicalizing both operands with `Path.resolve()` (or the PowerShell equivalent). Runners and temp directories may use an 8.3 alias such as `RUNNER~1` while code resolves the same directory to its long username; raw path-string equality is not a valid identity check.
...
- For a review builder, write intermediate package outputs below `generated/<feature>-review/.current-build/`, then replace the fixed `generated/<feature>-review/current/` directory as one unit. Never pass an `assets/gearwright/...` output path to a `graphics/review/*.py` builder. A manifest's `runtimePromotion: true` records that the approved reference may later be promoted by an explicit runtime change; it does not authorize the review generator to write runtime assets. The focused guard is `python -m unittest tests.graphics.test_lateral_drive_mount_review.LateralDriveMountReviewTests.test_candidates_are_review_only_and_use_one_managed_directory -v`.
