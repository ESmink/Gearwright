...
public sealed class BlockFluidPipe : Block
{
    ...
    public override bool OnBlockInteractStart(IWorldAccessor world, IPlayer byPlayer, BlockSelection blockSel)
    {
        ...
        if (!removing && !installingSprinkler && !installingWindow) return false;
        ...
        bool changed = removing
            ...
            : pipe.TryInstallAddon(blockSel.Face, held!, installingSprinkler, byPlayer);

        if (changed && !removing && byPlayer.WorldData.CurrentGameMode != EnumGameMode.Creative)
        {
            slot.TakeOut(1);
            slot.MarkDirty();
        }
    ...
    private static Cuboidf[] GetStateBoxes(IBlockAccessor blockAccessor, BlockPos pos)
    {
        ...
        {
            if (pipe.IsConnected(face)) AddConnectionBoxes(boxes, face);
        }
